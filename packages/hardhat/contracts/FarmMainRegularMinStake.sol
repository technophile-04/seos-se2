...
contract FarmMainRegularMinStake is IFarmMainRegular, BlockRetriever {
  ...
  modifier activeSetupOnly(uint256 setupIndex) {
    ...
    require(
      _setups[setupIndex].startBlock <= _blockNumber() && _setups[setupIndex].endBlock > _blockNumber(),
      "Invalid setup"
    );
    _;
  }
  ...
  function lazyInit(bytes memory lazyInitData) public returns (bytes memory extensionReturnCall) {
    ...
    (
      address extension,
      bytes memory extensionInitData,
      address rewardTokenAddress,
      bytes memory farmingSetupInfosBytes
    ) = abi.decode(lazyInitData, (address, bytes, address, bytes));
  ...
  function setFarmingSetups(FarmingSetupConfiguration[] memory farmingSetups) public override byExtension {
    for (uint256 i = 0; i < farmingSetups.length; i++) {
      _setOrAddFarmingSetupInfo(
        farmingSetups[i].info,
        farmingSetups[i].add,
        farmingSetups[i].disable,
        farmingSetups[i].index
      );
    }
  }
   ...
   */
  function position(uint256 positionId) public view override returns (FarmingPosition memory) {
    return _positions[positionId];
  }

  function setup(uint256 setupIndex) public view override returns (FarmingSetup memory, FarmingSetupInfo memory) {
    return (_setups[setupIndex], _setupsInfo[_setups[setupIndex].infoIndex]);
  }

  function setups() public view override returns (FarmingSetup[] memory) {
  ...
  function activateSetup(uint256 setupInfoIndex) public {
    require(
      _setupsInfo[setupInfoIndex].renewTimes > 0 && !_setups[_setupsInfo[setupInfoIndex].lastSetupIndex].active,
      "Invalid toggle."
    );
  ...
  function openPosition(FarmingPositionRequest memory request) public payable override returns (uint256 positionId) {
    ...
    require(
      _setups[request.setupIndex].startBlock <= _blockNumber() && _setups[request.setupIndex].endBlock > _blockNumber(),
      "Invalid setup"
    );
  ...
  function addLiquidity(
  ...
  ) public payable override activeSetupOnly(request.setupIndex) byPositionOwner(positionId) {
    ...
    _rewardPerTokenPerSetup[farmingPosition.setupIndex] +=
      (((_blockNumber() - chosenSetup.lastUpdateBlock) * chosenSetup.rewardPerBlock) * 1e18) /
      chosenSetup.totalSupply;
  ...
  function _withdrawReward(
    ...
    _rewardPerTokenPerSetup[farmingPosition.setupIndex] +=
      (((currentBlock - farmingSetup.lastUpdateBlock) * farmingSetup.rewardPerBlock) * 1e18) /
      farmingSetup.totalSupply;
    ...
    _retrieveGen2LiquidityAndFees(
      positionId,
      farmingPosition.tokenId,
      farmingPosition.uniqueOwner,
      liquidityToRemove,
      amount0Min,
      amount1Min,
      burnData
  ...
  function withdrawLiquidity(
    ...
    bytes memory burnData
  ) public byPositionOwner(positionId) {
  ...
  function _withdrawLiquidity(
    ...
    if (liquidityPoolTokenAmount == 0) {
      ...
      require(
        _setupsInfo[_setups[farmingPosition.setupIndex].infoIndex].minStakeable == 0,
        "Min stake: cannot remove partial liquidity"
      );
    }
  }
  ...
  function calculateFreeFarmingReward(uint256 positionId, bool isExt) public view returns (uint256 reward) {
    ...
    uint128 liquidityPoolTokenAmount = _getLiquidityPoolTokenAmount(farmingPosition.tokenId);
    reward =
      ((_rewardPerTokenPerSetup[farmingPosition.setupIndex] - _rewardPerTokenPaid[positionId]) *
        liquidityPoolTokenAmount) /
      1e18;
    if (isExt) {
      uint256 currentBlock = _blockNumber() < _setups[farmingPosition.setupIndex].endBlock
        ? _blockNumber()
        : _setups[farmingPosition.setupIndex].endBlock;
      uint256 lastUpdateBlock = _setups[farmingPosition.setupIndex].lastUpdateBlock <
        _setups[farmingPosition.setupIndex].startBlock
        ? _setups[farmingPosition.setupIndex].startBlock
        : _setups[farmingPosition.setupIndex].lastUpdateBlock;
      uint256 rpt = (((currentBlock - lastUpdateBlock) * _setups[farmingPosition.setupIndex].rewardPerBlock) * 1e18) /
        _setups[farmingPosition.setupIndex].totalSupply;
  ...
  function _setOrAddFarmingSetupInfo(FarmingSetupInfo memory info, bool add, bool disable, uint256 setupIndex) private {
    ...
    if (add) {
      require(
        farmingSetupInfo.liquidityPoolTokenAddress != address(0) && farmingSetupInfo.originalRewardPerBlock > 0,
      ...
      _setups[_farmingSetupsCount] = FarmingSetup(
        _farmingSetupsInfoCount,
        false,
        0,
        0,
        0,
        0,
        farmingSetupInfo.originalRewardPerBlock,
        0
      );
    ...
    if (setup.active) {
      ...
      if (_blockNumber() < setup.endBlock) {
        uint256 difference = info.originalRewardPerBlock < farmingSetupInfo.originalRewardPerBlock
          ? farmingSetupInfo.originalRewardPerBlock - info.originalRewardPerBlock
          : info.originalRewardPerBlock - farmingSetupInfo.originalRewardPerBlock;
  ...
  function _transferToMeAndCheckAllowance(
    FarmingSetup memory setup,
    FarmingPositionRequest memory request
  ) private returns (uint256 mainTokenPosition) {
    ...
    require(
      (_setupsInfo[setup.infoIndex].mainTokenAddress == tokens[0] ? tokenAmounts[0] : tokenAmounts[1]) >=
        _setupsInfo[setup.infoIndex].minStakeable,
      "Invalid liquidity."
    );
  ...
  function _addLiquidity(
  ...
  ) private returns (uint256 tokenId, uint128 liquidityAmount) {
    ...
    if (tokenId == 0) {
      ...
      (tokenId, liquidityAmount, amount0, amount1) = abi.decode(
        IMulticall(address(nonfungiblePositionManager)).multicall{value: ethValue}(data)[0],
        (uint256, uint128, uint256, uint256)
      );
    } else {
      data[0] = abi.encodeWithSelector(
        nonfungiblePositionManager.increaseLiquidity.selector,
      ...
      (liquidityAmount, amount0, amount1) = abi.decode(
        IMulticall(address(nonfungiblePositionManager)).multicall{value: ethValue}(data)[0],
    ...
    if (amount0 < request.amount0) {
      _safeTransfer(
        setupInfo.involvingETH && token0 == _WETH ? address(0) : token0,
        msg.sender,
        request.amount0 - amount0
      );
    }

    if (amount1 < request.amount1) {
      _safeTransfer(
        setupInfo.involvingETH && token1 == _WETH ? address(0) : token1,
        msg.sender,
        request.amount1 - amount1
      );
    }
  }
  ...
  function _updateFreeSetup(uint256 setupIndex, uint128 amount, uint256 positionId, bool fromExit) private {
    uint256 currentBlock = _blockNumber() < _setups[setupIndex].endBlock
      ? _blockNumber()
      : _setups[setupIndex].endBlock;
    if (_setups[setupIndex].totalSupply != 0) {
      uint256 lastUpdateBlock = _setups[setupIndex].lastUpdateBlock < _setups[setupIndex].startBlock
        ? _setups[setupIndex].startBlock
        : _setups[setupIndex].lastUpdateBlock;
      _rewardPerTokenPerSetup[setupIndex] +=
        (((currentBlock - lastUpdateBlock) * _setups[setupIndex].rewardPerBlock) * 1e18) /
        _setups[setupIndex].totalSupply;
    }
    // update the last block update variable
  ...
  function _toggleSetup(uint256 setupIndex) private {
    ...
    if (setup.active && wasActive) {
      ...
      _setups[_farmingSetupsCount].endBlock =
        _blockNumber() +
        _setupsInfo[_setups[_farmingSetupsCount].infoIndex].blockDuration;
  ...
  function _safeApprove(address erc20TokenAddress, address to, uint256 value) internal virtual {
    ...
    bytes memory returnData = _call(
      erc20TokenAddress,
      abi.encodeWithSelector(IERC20(erc20TokenAddress).approve.selector, to, value)
    );
  ...
  function _safeTransfer(address erc20TokenAddress, address to, uint256 value) internal virtual {
    ...
    bytes memory returnData = _call(
      erc20TokenAddress,
      abi.encodeWithSelector(IERC20(erc20TokenAddress).transfer.selector, to, value)
    );
  ...
  function _safeTransferFrom(address erc20TokenAddress, address from, address to, uint256 value) private {
    ...
    bytes memory returnData = _call(
      erc20TokenAddress,
      abi.encodeWithSelector(IERC20(erc20TokenAddress).transferFrom.selector, from, to, value)
    );
  ...
  function _ensureTransfer(uint256 amount) private returns (bool) {
    uint256 initialBalance = _rewardTokenAddress == address(0)
    ...
    uint256 actualBalance = _rewardTokenAddress == address(0)
      ? address(this).balance
      : IERC20(_rewardTokenAddress).balanceOf(address(this));
  ...
  function _retrieveGen2LiquidityAndFees(
    uint256 positionId,
    uint256 tokenId,
    address recipient,
    uint128 liquidityToRemove,
    ...
    if (liquidityToRemove > 0) {
      (decreasedAmount0, decreasedAmount1) = nonfungiblePositionManager.decreaseLiquidity(
  ...
  function _payFee(address tokenAddress, uint256 feeAmount) private returns (uint256) {
    ...
    return
      IFarmFactory(initializer).payFee{value: tokenAddress != address(0) ? 0 : feeAmount}(
        address(this),
        tokenAddress,
        feeAmount,
        ""
  ...
  function _collect(
    ...
    if (involvingETH) {
      ...
      data[2] = abi.encodeWithSelector(
        nonfungiblePositionManager.sweepToken.selector,
        token0 == _WETH ? token1 : token0,
        0,
        address(this)
      );
    ...
    (amount0, amount1) = abi.decode(
      IMulticall(address(nonfungiblePositionManager)).multicall(data)[0],
      (uint256, uint256)
    );
  }
}
...
interface IFarmFactory {
  ...
  ) external payable returns (uint256 feePaid);

  function burnOrTransferToken(
    address sender,
    bytes calldata permitSignature
  ) external payable returns (uint256 amountTransferedOrBurnt);
}
